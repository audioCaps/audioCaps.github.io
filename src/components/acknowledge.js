import React from "react"

const Acknowledge = () => (
  <div id="acknowlege" className="container mb-5">
    <h2>Acknowledgement</h2>
    <p>
      We thank SNU Vision & Learning Lab members and Yunseok Jang for helpful discussions.<br/>
      This work is supported by Kakao and Kakao Brain corporations and the in- ternational cooperation program by the NRF of Korea (NRF-2018K2A9A2A11080927).<br/>
      Gunhee Kim is the corresponding author.
    </p>
  </div>
)

export default Acknowledge
